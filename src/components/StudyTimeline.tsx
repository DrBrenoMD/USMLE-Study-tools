...
export const StudyTimeline: React.FC<StudyTimelineProps> = ({
    ...
    switch (type) {
      case 'qbank':
        return {
          ...
          barBg: 'bg-blue-100 dark:bg-blue-900/50 border-blue-300 text-blue-900',
          badge: 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200',
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200/90 dark:border-gray-700/90 rounded-2xl p-5 sm:p-6 shadow-sm space-y-5">
      {/* Cabeçalho */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 dark:border-gray-800 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-200 flex items-center justify-center text-blue-600 dark:text-blue-400">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            Hoje: <strong className="text-gray-900 dark:text-gray-100">{format(today, 'dd/MM/yyyy')}</strong>
          </div>
          {plan.targetFinishDate && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-200 text-blue-800 dark:text-blue-200">
      ...
      <div className="space-y-3 pt-1">
        {/* Marcadores de Datas */}
        <div className="relative h-5 text-[11px] font-medium text-gray-500 dark:text-gray-400 select-none">
          <span className="absolute left-0 text-emerald-700 font-semibold flex items-center gap-1">
            📍 Hoje ({format(today, 'dd/MM')})
          </span>
          {plan.targetFinishDate && (
            <span
              className="absolute -translate-x-1/2 text-blue-700 dark:text-blue-300 font-medium hidden sm:inline-block"
                ...
                <div
                  ...
                  className={`group relative rounded-xl border transition-all cursor-pointer p-3 ${
                    isSelected
                      ...
                      : isCompleted
                      ? 'bg-white/80 dark:bg-gray-800/80 border-gray-200 dark:border-gray-700 opacity-90'
                      : item.activeNow
                      ? 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:border-gray-600 shadow-xs'
                      : 'bg-white/60 dark:bg-gray-800/60 border-dashed border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:border-gray-600'
                  }`}
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-1.5">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {isCompleted ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200">
                    ...
                    {/* Datas do período */}
                    <div className="text-xs text-gray-600 dark:text-gray-400 flex items-center gap-1.5 sm:gap-2">
                  ...
                  <div className="relative w-full h-5 bg-gray-100 dark:bg-gray-800 rounded-md overflow-hidden border border-gray-200 dark:border-gray-700 my-1">
                    {item.frequency !== 'daily' && item.scheduledDates && item.scheduledDates.length > 0 ? (
                      <>
                        ...
                        <div className="absolute inset-0 flex items-center px-2 text-[10px] font-semibold text-gray-700 dark:text-gray-300 z-10 pointer-events-none drop-shadow-sm">
                          {isCompleted
                            ? '100% Concluído'
                            : item.activeNow
                      ...
                      <div
                        className={`absolute top-0 bottom-0 rounded-[4px] transition-all border flex items-center px-2 text-[10px] font-semibold whitespace-nowrap overflow-hidden ${
                          isCompleted
                            ...
                            : item.activeNow
                            ? `${color.barBg}`
                            : 'bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 border-dashed'
                        }`}
                        style={{
                          left: `${startPct}%`,
                          width: `${widthPct}%`,
                  ...
                  <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-800 flex flex-wrap items-center justify-between gap-2 text-xs">
                      ...
                      {isCompleted ? (
                        ...
                        <>
                          <span className="flex items-center gap-1 text-gray-900 dark:text-gray-100 font-medium">
                            ...
                            Carga Diária: <strong className="text-blue-700 dark:text-blue-300">{item.dailyAmount} {item.unit}/dia</strong>
                          </span>
