...
bad_block = """                            {/* 5. Metas Diárias Fixas por Dia da Semana (Avançado) */}
...
print(f"Found {len(parts) - 1} instances of the bad block to fix")

# We want to keep the FIRST instance but fix its syntax (it should be `)} </div>` instead of `</div> )}` at the end?)
# Actually, the FIRST instance was originally:
#                             </div>
#                           )}
#                         </div>

# The 2nd and 3rd instances were originally the same target string:
#                             </div>
#                           )}
#                         </div>
...
# First instance: keep it but fix syntax. The syntax was:
#                            </div> (ends #4)
#                           )} (ends isExpanded)
#                         </div> (ends the whole card)
# Since I added #5 inside isExpanded, it should be:
#                            </div> (ends #4)
#                            {/* 5. ... */}
#                            {resource.frequency === 'daily' && ... }
#                          </div> (ends Gaveta grid!)
#                          )} (ends isExpanded)
#                         </div> (ends the whole card)
...
new_content = parts[0] + fixed_first + parts[1] + original_target + parts[2] + original_target + parts[3]
